/* app.js — géolocalisation, carte Leaflet plein écran, panneau coulissant,
   liste, couleurs et édition. */

import { Storage, yearOf, formatDate } from './storage.js';
...
// Pas de boutons +/- : on zoome au pincement, l'interface flotte par-dessus.
const map = L.map('map', { zoomControl: false }).setView([46.6, 2.5], 5);
...
let meMarker = null;
let mePosition = null; // dernière position connue [lat, lng]
...
function setStatus(msg, kind) {
  ...
  toastTimer = setTimeout(() => { els.toast.hidden = true; }, kind === 'error' ? 5000 : 3500);
}
...
function focusSpot(s) {
  setSheet('peek'); // replie le panneau pour dégager la carte
...
/* --- Panneau coulissant (peek / half / full) --- */
let sheetState = 'half';

// Décalages translateY (px) de chaque état, recalculés à la volée.
function sheetOffsets() {
  const h = els.sheet.offsetHeight;
  // Replié : on laisse visibles la poignée + le bouton marquer + la palette.
  const peekVisible = els.sheetHeader.offsetTop + els.sheetHeader.offsetHeight;
  return { full: 0, half: Math.round(h * 0.5), peek: h - peekVisible };
}

function setSheet(state, animate = true) {
  sheetState = state;
  const o = sheetOffsets();
  els.sheet.style.transition = animate ? 'transform 0.3s cubic-bezier(0.2, 0.9, 0.3, 1)' : 'none';
  els.sheet.style.transform = `translateY(${o[state]}px)`;
  els.sheet.dataset.state = state;
  document.body.dataset.sheet = state;
  // Hauteur visible du panneau (bouton position) et de l'état replié (attribution OSM).
  document.documentElement.style.setProperty('--sheet-visible', `${els.sheet.offsetHeight - o[state]}px`);
  document.documentElement.style.setProperty('--peek-visible', `${els.sheet.offsetHeight - o.peek}px`);
}
...
function initSheetDrag() {
  ...
  els.sheetHandle.addEventListener('pointerdown', (e) => {
    ...
    els.sheetHandle.setPointerCapture(e.pointerId);
  });

  els.sheetHandle.addEventListener('pointermove', (e) => {
    if (startY == null) return;
    const o = sheetOffsets();
    const y = Math.min(o.peek, Math.max(o.full, startOffset + e.clientY - startY));
  ...
  const release = (e) => {
    ...
    startY = null;
    // Aimante à l'état le plus proche.
    const o = sheetOffsets();
    let best = sheetState;
    let bestDist = Infinity;
    for (const [state, offset] of Object.entries(o)) {
      const d = Math.abs(offset - y);
      if (d < bestDist) { bestDist = d; best = state; }
    }
    setSheet(best);
  };
  els.sheetHandle.addEventListener('pointerup', release);
  els.sheetHandle.addEventListener('pointercancel', () => { startY = null; setSheet(sheetState); });

  window.addEventListener('resize', () => setSheet(sheetState, false));
}
...
function locateMe() {
  if (!mePosition) {
    setStatus('Position inconnue pour l\'instant. Autorise la localisation et patiente.', 'error');
    return;
  }
  setSheet('peek');
  map.setView(mePosition, Math.max(map.getZoom(), 15));
}
